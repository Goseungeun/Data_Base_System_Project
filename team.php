        ...
        <table id="TeamRakingTable" class="table table-striped text-center my-3 align-middle">
            ...
            <tbody>
              <?php
                ...
                $sql = "SELECT * FROM team ORDER BY ranking";
                $result = mysqli_query($conn,$sql);
                while($row = mysqli_fetch_array($result)){
                  $url = "./team_info.php?teamname=";
                  $teamname = $row['teamname'];
                  $f_url = $url.$teamname;
                  echo '<tr><td>'.$row['ranking'].'</td><td><img class = "logo" src="./logo/'.$row['emblem'].'"</td><td>'.$row['teamname'].'</td><td><a href = "'.$f_url.'">더보기</a></td></tr>';
                }
                ?>
            </tbody>
          </table>
